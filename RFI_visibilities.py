...
class sim():
 ...
 #__________________________________ADDITION OF NOISE_______________________________________ 
      ...
      def adding_noise(self):
          ...
          #Plotting Amplitude
          plt.subplot(311)
          Amp = plt.imshow(abs(D), aspect = "auto")           
          plt.ylabel("Frequency", fontsize=12)
          ...
          #Plotting Phase
          plt.subplot(312)       #Plots one row and two columns,second subplot 
          Phase=plt.imshow(np.angle(D), aspect = "auto")      
          plt.ylabel("Frequency", fontsize=12)
          plt.colorbar()
          plt.subplots_adjust(hspace=.030)
          ax1 = plt.gca()
...
def main():
    ...
    return num_sources,fov,pareto_number,num_channels_to_corrupt,SNR,SNR_high
    

if __name__ =="__main__":
   num_sources,fov,pareto_number,num_channels_to_corrupt,SNR,SNR_high=main()
   s = sim()
   s.latitude()
   s.declination()
   s.lam()
